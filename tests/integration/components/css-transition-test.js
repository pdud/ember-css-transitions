...
module('Integration | Component | transition group', function(hooks) {
  ...
  module('Enter and leave via transition classes', function() {
    let testCases = [{
      ...
      template: hbs`
        {{#if this.show}}
          <div id="my-element" {{css-transition enterClass="opacity-0" enterActiveClass="duration-500 opacity-100" leaveClass="opacity-100" leaveActiveClass="duration-500 opacity-0" didTransitionIn=this.didTransitionIn didTransitionOut=this.didTransitionOut}}>
            <p class="content">Çup?</p>
          </div>
        {{/if}}
      `
      ...
      template: hbs`
        {{#if this.show}}
          <MyComponent id="my-element" {{css-transition enterClass="opacity-0" enterActiveClass="duration-500 opacity-100" leaveClass="opacity-100" leaveActiveClass="duration-500 opacity-0" didTransitionIn=this.didTransitionIn didTransitionOut=this.didTransitionOut}}>
            <p class="content">Çup?</p>
          </MyComponent>
        {{/if}}
      `
      ...
      template: hbs`
        {{#if this.show}}
          <GlimmerComponent id="my-element" {{css-transition enterClass="opacity-0" enterActiveClass="duration-500 opacity-100" leaveClass="opacity-100" leaveActiveClass="duration-500 opacity-0" didTransitionIn=this.didTransitionIn didTransitionOut=this.didTransitionOut}}>
            <p class="content">Çup?</p>
          </GlimmerComponent>
        {{/if}}
      `
    ...
    testCases.forEach((i) => {
      ...
      test(`enter and leave transitions work (${i.name})`, async function(assert) {
        ...
        assert.dom('#my-element').hasClass('opacity-100', '-enter-active is applied');
        assert.dom('#my-element').hasClass('duration-500', '-enter-active is applied');
        ...
        assert.dom('#my-element_clone').hasClass('opacity-0', '-leave-active is applied after `afterRender` and a browser repaint on clone');
